...
# ---------------------------------------------------------------------------
# Sanitizers (ORM row → plain dict for responses)
# ---------------------------------------------------------------------------
...
@app.post("/api/cars/")
async def create_car(payload: CarPayload, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    vehicle = Vehicle(
...
async def update_car(vehicle_id: int, payload: CarPayload, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
...
async def delete_car(vehicle_id: int, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    vehicle = db.query(Vehicle).filter(Vehicle.id == vehicle_id).first()
    if not vehicle:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Vehicle not found")
    db.delete(vehicle)
